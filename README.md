...
If the installation was successful then your job can be submitted to the cluster with `$ sbatch job.slurm`. If the installation failed and packages were downloaded then you should remove those packages before proceeding (see contents of `~/.conda`). If both the pip and conda procedures failed then continue reading or see the [Getting Help](#getting_help) section at the bottom of this page.
